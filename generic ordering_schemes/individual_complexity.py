...
#sort data by connectivity and neighbour connectivity where edges are between vertices of the same class with a distance lower than a threshold
def order_by_network(df, keep_scores = False):
    threshold = 0.005*abs(df.iloc[:, 0:df.shape[0]-2].max().max()-df.iloc[:, 0:df.shape[0]-2].min().min())  #0.15 is recommended by literature but it is too expensive
    edges = np.empty((df["class"].max()), dtype = object)
    for class_no in range(len(edges)):
        edges[class_no] = []
        ...
        for comparison_row_no in range(len(df)):
            if df["class"].iat[datum_row_no] == df["class"].iat[comparison_row_no] and datum_row_no != comparison_row_no: #disallow edges connecting vetices to themselves or to vertices belonging to a different class
                ...
                if distance < threshold:
                    ...
                    while edge_no < len(edges[datum_class-1]) and not edge_already_exists: #if an edge already exists, do not store it as order does not matter
                        if datum_row_no in edges[datum_class-1][edge_no] and comparison_row_no in edges[datum_class-1][edge_no]:
                            edge_already_exists = True
                        edge_no += 1
                    if not edge_already_exists:
                        edges[datum_class-1].append((datum_row_no, comparison_row_no))
    ...
    for datum_row_no in range(len(df)):
        ...
        if df["checked"].iat[datum_row_no] is True:
            ...
            for edge in edges[datum_class-1]:
                if edge[0] == datum_row_no:
                    ...
                    if df["checked"].iat[edge[0]] is True:
...
def make_binary_data_sets(df):
    ...
    binary_dfs = []
    for class_no in range(1, num_classes):
